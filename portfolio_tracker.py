...
def display_portfolio(portfolio: dict) -> None:
    ...
    """
    if not portfolio:
        print("\n  Your portfolio is empty. Add some coins!\n")
        return
...
def fetch_live_prices() -> bool:
    ...
    try:
        ...
        if response.status_code == 429:
            ...
            print("  Using cached prices. Try again in a minute.\n")
            return False

        if response.status_code == 401:
            print("  API key invalid or missing.")
            print("  Check your COINGECKO_API_KEY and try again.\n")
            return False
...
def remove_coin(portfolio: dict) -> None:
    ...
    """
    if not portfolio:
        print("  Nothing to remove -- portfolio is empty.\n")
        return
